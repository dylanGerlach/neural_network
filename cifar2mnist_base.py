...
from cifar2mnist_solution import *  # this is the file that you need to create.
...
pretrained_model_file = "cifar10_e20_b128_6.keras"
model.save(pretrained_model_file)
...
(mnist_tr_inputs, mnist_tr_labels), (mnist_test_inputs, mnist_test_labels) = (
    dataset.load_data()
)
...
for c in range(1, num_classes):
    small_indices = np.concatenate(
        (small_indices, class_indices[c][0:samples_per_class])
    )
...
# You need to write this function
refined_model = load_and_refine(
    pretrained_model_file, small_tr_inputs, small_tr_labels, batch_size, epochs
)

refined_model_file = "refined_mnist100_e40_b32.keras"
refined_model.save(refined_model_file)
